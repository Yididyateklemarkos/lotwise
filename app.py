...
from models import (db, User, VerificationDocument, Listing, ListingPhoto, SourcingRequest, Event,
...
def signup():
    if request.method == "POST":
        ...
        if not email or not password or not first_name or not last_name or account_type not in ("supplier", "buyer"):
        ...
        login_user(user)
        flash("Account created. Browse the marketplace while your verification is reviewed.", "success")
        return redirect(url_for("browse"))
...
def update_profile_name():
    ...
    current_user.company_name = f"{first_name} {last_name}"
    db.session.commit()
...
def _delete_connection_data_for_inquiries(inquiries):
    """Remove contact records before deleting their listing/request parent.
    Uses synchronize_session='fetch' (not False) so SQLAlchemy's in-memory
    session stays consistent with these bulk deletes — otherwise the
    subsequent listing/request delete tries to update rows that were just
    bulk-removed and throws a StaleDataError."""
    inquiry_ids = [inquiry.id for inquiry in inquiries]
    if not inquiry_ids:
        return
    connections = Connection.query.filter(Connection.inquiry_id.in_(inquiry_ids)).all()
    connection_ids = [connection.id for connection in connections]
    if connection_ids:
        Message.query.filter(Message.connection_id.in_(connection_ids)).delete(
            synchronize_session="fetch"
        )
        Connection.query.filter(Connection.id.in_(connection_ids)).delete(
            synchronize_session="fetch"
        )
    Inquiry.query.filter(Inquiry.id.in_(inquiry_ids)).delete(
        synchronize_session="fetch"
    )


def _delete_listing(listing):
    _delete_connection_data_for_inquiries(listing.inquiries)
...
def _delete_sourcing_request(sourcing_request):
    _delete_connection_data_for_inquiries(sourcing_request.inquiries)
    for photo in sourcing_request.photos:
        photo_path = os.path.join(app.config["UPLOAD_FOLDER"], photo.file_path)
        if os.path.exists(photo_path):
            os.remove(photo_path)
    db.session.delete(sourcing_request)
...
# ---------------------------------------------------------------------------
# Legacy track-record URL
# ---------------------------------------------------------------------------
@app.route("/dashboard/track-record", methods=["GET", "POST"])
@login_required
def track_record():
    flash("Track record is no longer part of Lotwise. Connections now exchange contact details directly.", "info")
    return redirect(url_for("dashboard"))
...
@app.route("/dashboard/requests/<int:request_id>/delete", methods=["POST"])
@login_required
def delete_own_request(request_id):
    sourcing_request = SourcingRequest.query.get_or_404(request_id)
    if sourcing_request.user_id != current_user.id:
        abort(403)
    _delete_sourcing_request(sourcing_request)
    db.session.commit()
    flash("Sourcing request deleted.", "info")
    return redirect(url_for("dashboard"))


# ---------------------------------------------------------------------------
# Marketplace (the feed is visible to logged-in applicants; details and
# actions remain restricted to verified, paid members)
# ---------------------------------------------------------------------------
...
def browse():
    ...
    return render_template(
        ...
        access_unlocked=current_user.is_active_member(),
    )
...
def events():
    access_unlocked = current_user.is_active_member()
    event_list = Event.query.filter_by(is_published=True).filter(
        Event.starts_at >= datetime.utcnow()
    ).order_by(Event.starts_at.asc()).all() if access_unlocked else []
...
def public_profile(user_id):
    ...
    listings = Listing.query.filter_by(
        user_id=profile.id, is_active=True, is_sold=False
    ).order_by(Listing.created_at.desc()).all()
    requests_list = SourcingRequest.query.filter_by(
        user_id=profile.id, is_active=True
    ).order_by(SourcingRequest.created_at.desc()).all()
    return render_template("dashboard/profile.html", profile=profile,
                           listings=listings, requests_list=requests_list)
...
def connection_thread(connection_id):
    ...
    return render_template("dashboard/connection_thread.html", connection=conn,
...
@verified_required
def billing_upgrade():
    """Shows the 3 tier options. Each plan goes directly to Whop checkout."""
...
def billing_pay(tier):
    """Compatibility entry point: plan selections go straight to Whop."""
    prices = TIER_PRICE_USD if current_user.account_type == "supplier" else BUYER_TIER_PRICE_USD
    if tier not in prices:
        abort(404)
    return redirect(url_for("billing_pay_whop", tier=tier))
...
def admin_approve(user_id):
    user = User.query.get_or_404(user_id)
    doc_types = {document.doc_type for document in user.documents}
    if not {"national_id", "passport"}.issubset(doc_types):
        flash("Approval blocked: both a National ID and Passport are required.", "error")
...
def admin_events():
    ...
    events_list = Event.query.order_by(Event.starts_at.asc()).all()
    return render_template("admin/events.html", events_list=events_list)
...
with app.app_context():
    db.create_all()
    # db.create_all() does not alter an existing users table. Keep imported
    # SQLite/Postgres databases compatible with the new profile name fields.
    from sqlalchemy import inspect, text
    user_columns = {column["name"] for column in inspect(db.engine).get_columns("users")}
    for column_name, column_type in (("first_name", "VARCHAR(100)"), ("last_name", "VARCHAR(100)")):
